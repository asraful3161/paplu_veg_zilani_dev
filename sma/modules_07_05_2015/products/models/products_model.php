...
class Products_model extends CI_Model
{
		...
		if($photo == NULL) {
			...
			$productData = array(
				...
				'image'   			=> $photo
                 ...
                 $previousData = $this->getProductByID($id); 
                 $preQquantity = $previousData->quantity; 
                 $preUnit = $previousData->unit;
                 
                 $unitDifference = $data['unit'] - $preUnit; 
		 $currentQuantity = $preQquantity + $unitDifference;
		
		$this->db->where('id', $id);
		if($this->db->update('products', $productData)) {
                    
                        $prD = $this->getProductByID($id);  
		        $aQTY = $data['unit'] + $prD->quantity; 
                        $this->db->update('products', array('available_quantity' => $aQTY, 'quantity' => $currentQuantity), array('id' => $id));     
    ...
    public function resetAllProductStock()
    {
           $discountData = array(
				'unit'	     	                => 0.00,
                                'quantity'	     	        => 0.00,
           ...
           $result=$this->db->update('products', $discountData);
    ...
    ////////////////////////////////For Promotion///////////////////////////////////////
     ...
     public function getAllProtion() 
	{       

		$myQuery ='SELECT promotion.*,products.name
                            FROM promotion
                            LEFT JOIN products ON promotion.product_id=products.id';  
		...
		if($q->num_rows() > 0) {
         ...
         public function checkDuplicatePromotion($product_id )  
	{ 
        ...
        public function addPromotion($data)
	{
		...
		if($this->db->insert('promotion', $promotionData)) {
			return true;
		} else {
			return false;
		}
         ...
         public function updatePromotion($promotion_id, $data = array())
	{ 
		...
		if($this->db->update('promotion', $promotionData)) {
			return true;
		} else {
			return false;
		}
        ...
        public function getDuplicatePromotionData($product_id) 
	{
            

	     $q = $this->db->get_where('promotion', array('product_id' => $product_id), 1); 
		  if( $q->num_rows() > 0 )
		  {
			return $q->row();
		  } 
        ...
        public function updateProductPromotion($promotion_id, $data = array())
	{ 
	...
	{ 
            ...
            $myQuery ="SELECT promotion.*,  products.name as product_name
                       FROM promotion
                       LEFT JOIN products ON products.id=promotion.product_id
	...
	{
		if($this->db->delete('promotion', array('promotion_id' => $id))) { 
			return true;
		}
	return FALSE;
	}
