...
//Add new pos sale
    ...
    function index() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $ordernote = $this->input->post('order_note');
            $count = $this->input->post('count');
            $count = $count - 1;
            ...
            $cusPreBalance = $this->pos_model->getCustomerBalancebyID($customer_id);
            //$customer_Pre_Balance=$cusPreBalance->customer_balance;
            $customer_Pre_Balance=$cusPreBalance->total_balance_amount; 
            $this->session->set_userdata('customer_Pre_Balance', $customer_Pre_Balance); 
            ...
            for ($i = 1; $i <= 500; $i++) {
                if ($this->input->post($quantity . $i) && $this->input->post($product . $i) && $this->input->post($unit_price . $i)) {
                    ...
                    $prweight = $this->pos_model->getCustomerProductWeight($product_details->id);
            ...
            $keys = array("product_id", "product_code", "product_name", "product_unit", "tax_rate_id", "tax", "quantity", "sale_product_weight", "unit_price", "gross_total", "val_tax", "serial_no", "discount_val", "discount", "discount_id");

            $items = array();
            foreach (array_map(null, $product_id, $product_code, $product_name, $product_unit, $tax_rate_id, $tax, $inv_quantity, $sale_product_weight,  $inv_unit_price, $inv_gross_total, $val_tax, $serial, $val_ds, $discount, $dsID) as $key => $value) {
            ...
            $gTotal = $inv_total_no_tax + $total_tax + $val_tax2 - $val_discount;
                 ...
                 $paidValue=$this->input->post('paid_val');
                 
            
            

            echo $this->input->post('ordernote');
            $saleDetails = array(
        ...
        if ($this->form_validation->run() == true && !empty($items)) {
            if ($suspend) {
                ...
                if ($saleID = $this->pos_model->addSale($saleDetails, $items, $warehouse_id, $did, $paidValue, $customer_id, $gTotal )) {
...
//    }
    function price($code = NULL, $custid = NULL) {
        if ($this->input->get('code')) {
            $code = $this->input->get('code');
        }
        ...
        $cus_discount = $this->pos_model->getDiscountAmountByID ($custid, $product_id); 
        
        if($cus_discount!=0){
            $dis_amount= $cus_discount->discount_amount; 
        ...
        $data = array('price' => $dis_price, 'original_price' => $price, 'discount_amount' => $dis_amount, 'name' => $name, 'code' => $code, 'tax_rate' => $tax_rate);
        echo json_encode($data);
    }
    ...
    function ajaxproducts($category_id = NULL) {
        ...
        foreach ($products as $product) {
            ...
            if ($product->image == 'no_image.jpg') {
                $prods .= "
			<button id=\"product-" . $category_id . $count . "\" type=\"button\" value='" . $product->code . "' class=\"green\" ><i><img src=\"assets/uploads/thumbs/default.png\"></i><span><span>" . $product->name . "</span></span><span>Qty: " . $product->quantity . "</span></button>";
            } else {
                $prods .= "
			<button id=\"product-" . $category_id . $count . "\" type=\"button\" value='" . $product->code . "' class=\"green\" ><i><img src=\"assets/uploads/thumbs/" . $product->image . "\"></i><span><span>" . $product->name . "</span></span><span>Qty: " . $product->quantity . "</span></button>";
            }
            $pro++;
        }
...
//view inventory as html page
  ...
  /*  function view_invoice() {
        ...
        //print_r($data['rows']['product_id']); exit;   
...
*/
  ...
  /*  function view_invoice() {
        ...
        $data['customer_Pre_Balance'] = $this->session->userdata('customer_Pre_Balance'); 
        $data['cusBalance'] = $this->pos_model->getCustomerBalancebyID($customer_id);           
        
        $data['rows'] = $this->pos_model->getAllInvoiceItems($customer_id, $sale_id);
        ...
        function view_invoice() {
              ...
              $data['message'] = (validation_errors() ? validation_errors() : $this->session->flashdata('message'));
               ...
               $data['rows'] = $this->pos_model->getAllInvoiceItems_old($sale_id);

               $inv = $this->pos_model->getInvoiceBySaleID_old($sale_id);
               $biller_id = $inv->biller_id;
               $customer_id = $inv->customer_id;
               $invoice_type_id = $inv->invoice_type;
               $data['biller'] = $this->pos_model->getBillerByID($biller_id);
    ...
    function view_invoice_new() {
       ...
       // $cusDiscount = $this->pos_model->getCustomerProductDiscount($customer_id, $product_id); 
        
       //print_r($cusDiscount);exit;   
        
        
        $arr = $this->pos_model->getAllInvoiceItems($customer_id, $sale_id);
...
//        echo "</pre>";
        ...
        foreach ($arr1 as $key => $value) { 
            $productcode = $key;
            $quan = $value;
            $productsarr[]= $this->pos_model->getOrderedProductByCodewithQuan($sale_id, $productcode, $quan, $customer_id); 
          }
//          echo "<pre>"; 
//          print_r($productsarr);  
//          echo "</pre>";
        $data['rows1'] = $productsarr; 
        //print_r(json_encode($data['rows1']));
    ...
    function getSuspendedSales() {
        ...
        $this->datatables->add_column("Actions", "<center><a class=\"tip\" title='" . $this->lang->line("add_to_pos") . "' href='index.php?module=pos&amp;suspend_id=$1'><i class=\"icon-plus-sign\"></i></a> <a class=\"tip\" title='" . $this->lang->line("delete_suspended_sale") . "' href='index.php?module=pos&amp;view=delete&amp;id=$1' onClick=\"return confirm('" . $this->lang->line('alert_x_sale') . "')\"><i class=\"icon-trash\"></i></a></center>", "id")
                ->unset_column('id');
    ...
    function getCustomerBalance(){  
        $selectcustid= $sale_id = $this->input->post('selectedcusid');
        $cusBalance = $this->pos_model->getCustomerBalancebyID($selectcustid);   
        //$customer_balance = $cusBalance->customer_balance; 
        $customer_balance = $cusBalance->total_balance_amount;   
        
        $data = array('balance' => $customer_balance); 
        echo json_encode($data);
    }   
