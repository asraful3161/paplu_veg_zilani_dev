    ...
    $(document).ready(function () {
        ...
        $('#code').keydown(function (e) {
            ...
            if (e.keyCode == 13) {
                ...
                newTr.html('<td><input class="nade-field" name="product' + count + '" type="hidden" value="' + item_code + '"><input class="span5 tran" style="text-align:left;" name="item' + count + '" type="text" value="' + item_name + ' (' + item_code + ')"></td><?php
if (PRODUCT_SERIAL) {
...
?>
        ...
        function getExistProductRow(prCode){
             var allPrcodes= $("#dyTable .nade-field").map(function() {
                return $(this).val();
             }).get();
             var ret = jQuery.inArray(prCode, allPrcodes);
             console.log(ret);
             if(ret=="-1"){
                 return false;
             } else {
            ...
            select: function (event, ui) {
                ...
                $.ajax({
                    ...
                    success: function (data) { 
                        ...
                        var itemty = data.product_type;
                        checkExist = getExistProductRow(item_code);
                ...
                var newTr = $('<tr id="row_' + count + '"></tr>');
                newTr.html('<td><input class="nade-field" name="product' + count + '" type="hidden" value="' + item_code + '"><input class="span5 tran" style="text-align:left;" name="item' + count + '" type="text" value="' + item_name.replace(/"/g, "&#034;").replace(/'/g, "&#039;") + ' (' + item_code + ')"></td><?php
if (PRODUCT_SERIAL) {
    echo '<td><input class="span2 tran2" name="serial\'+ count +\'" type="text" value=""></td>';
}
if (DISCOUNT_OPTION == 2) {
...
?><td><input class="input-block-level text-center" name="quantity' + count + '" id="quantity-' + count + '" type="text" value="1" onClick="this.select();"></td>\n\
<td><input class="span2 tran" style="text-align:right;" name="unit_price' + count + '" id="price-' + count + '" type="text" value="' + item_price + '"></td>\n\
<td>\n\
<input class="input-block-level text-center" '+readOn+' name="tweight' + count + '" id="tweight-' + count + '" type="text" value="' + item_weight + '">\n\
<input class="input-block-level text-center" '+readOn+' name="var_poductnote' + count + '" id="var_poductnote_' + count + '" type="text" value="">\n\
</td>\n\
...
?><td><input class="input-block-level text-center" name="quantity' + count + '" type="text" value="1" onClick="this.select();"></td>\n\
<td><input class="span2 tran" style="text-align:right;" name="unit_price' + count + '" type="text" value="' + item_price + '"></td>\n\
<td>\n\
<input class="input-block-level text-center" '+readOn+' name="tweight' + count + '" id="tweight-' + count + '" type="text" value="' + item_weight + '">\n\
<input class="input-block-level text-center" '+readOn+' name="var_poductnote' + count + '" id="var_poductnote_' + count + '" type="text" value="">\n\
</td>\n\
...
<?php
$attrib = array('class' => 'form-horizontal', 'id' => 'addSale_form');
...
<div class="control-group table-group">
    ...
    <div class="controls table-controls">
        <table id="dyTable" class="table items table-striped table-bordered table-condensed table-hover">
            ...
            <th class="span2">Total Weight</th>
...
//                echo "</pre>";
                foreach ($inv_products as $prod) {
                    ...
                    echo '<tr id="row_' . $r . '"><td><input class="nade-field" name="product' . $r . '" type="hidden" value="' . $prod->product_code . '"><input class="span5 tran" style="text-align:left;" name="item' . $r . '" type="text" value="' . str_replace(array('"', '\''), array('&#034;', '&#039;'), $prod->product_name) . ' (' . $prod->product_code . ')"></td>';
                    if (PRODUCT_SERIAL) {
                    ...
                    if ($prod->product_type == "Variable") {
                        //echo $prod->product_type;
                        $varprodfracks = $prod->variprod_note;
                        
                        if(!empty($varprodfracks)){
                            $varprodfracksarr = explode("+", $varprodfracks);
                            if(sizeof($varprodfracksarr)>0){
                                $readonlyrev = 'readonly';
                            }
                        } else {
                            $readonlyrev = '';
                        } 
                    ...
                    echo '<td><input class="input-block-level text-center" name="quantity' . $r . '" id="quantity-' . $r . '" type="text" ' . $readonlyrev . ' value="' . $prod->quantity . '"></td>'
                    . '<td><input class="span2 tran" style="text-align:right;" name="unit_price' . $r . '" name="price-' . $r . '" type="text" value="' . $prod->unit_price . '"></td>'
                    . '<td>'
                    . '<input class="input-block-level text-center" id="tweight-' . $r . '" name="tweight-' . $r . '" type="text" ' . $readonly . ' value="' . $prod->sale_product_weight . '">'
                    . '<input class="input-block-level text-center" id="var_poductnote_' . $r . '"  name="var_poductnote' . $r . '"  type="text" value="' . $prod->variprod_note . '" ' . $readonly . ' />'
                    //. '<textarea placeholder="Weight Breakdown" class="span12 text-center style="text-align:right;" name="var_poductnote'.$r.'" '.$readonly.'>' . $prod->variprod_note. '</textarea>'
                    . '</td>'
                    . '<td><i class="icon-trash tip del" id="' . $r . '" title="Remove this Item" style="cursor:pointer;" data-placement="right"></i></td></tr>';
                    $r++;
                }
                ?>
            </tbody>
...
//                sum += parseInt(res[l]); 
//            }
//            var getID = currentField.context.id;
//            var nRes = getID.split("_");
//            var tweightID = nRes[2];
//            //$("#tweight-"+tweightID).val(sum);
//            $("#tweight-"+tweightID).prop("value", sum);
//            console.log(sum);
//            console.log('currentField ID:: ', currentField.context.id);
//        }
//    };
//    var ar = JSON.parse('</?php //echo json_encode($inv_products) ?>');
//    console.log(ar);
//    for (var i = 0; i < ar.length; i++) {
//        var prqnt = ar[i].quantity;
//        var maskqn = '';
//        console.log(prqnt);
//        for (var j = 0; j < prqnt; j++) {
//            maskqn += '01+';
//        }
//        console.log(maskqn);
//        $("#var_poductnote_" + (i + 1)).mask(maskqn, options);
//    }
</script>