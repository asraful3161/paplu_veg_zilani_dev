...
class Pos_model extends CI_Model
{
   ...
   public function categories_count() {
	...
	/* public function getAllInvoiceItems($sale_id) 
	{
		...
		if($q->num_rows() > 0) {
         ...
         public function getCustomerProductDiscount($customerid,$productid){ 
               
                //SELECT `discount_amount` FROM `customer_discount` WHERE `customer_id`=1 AND `product_id`=11
                
                $this->db->get("customer_discount.*"); 
                $this->db->from("customer_discount"); 
        ...
        public function getAllInvoiceItems($customerid, $sale_id) 
        {
         //$q = $this->db->get_where('sale_items', array('sale_id' => $sale_id));
         ...
         if($q->num_rows() > 0) { 
                           foreach (($q->result()) as $row) {
                               ...
                               $discount = $this->getCustomerProductDiscount($customerid, $productid);

                               $row->customer_discount = $discount;
	...
	public function addSale($saleDetails = array(), $items = array(), $warehouse_id, $sid = NULL,  $paidValue, $customer_id, $gTotal )
	{
		...
		$saleData = array(
			...
			'order_note'			=> $saleDetails['order_note'],
		...
		if($this->db->insert('sales', $saleData)) {
		     $sale_id = $this->db->insert_id();
                     ...
                     $cusBalance = $this->pos_model->getCustomerBalancebyID($customer_id);     
                     $customer_balance = $cusBalance->customer_balance;
                     $payableAmount = $cusBalance->payable_amount;
                     $date = $cusBalance->date;  
                     
                     $salesBalanceData = array(
                            'customer_balance'   => $customer_balance, 
                            );
                     
                     $this->db->update('sales', $salesBalanceData, array('id' => $sale_id));
                     
                     $present_balance = ($customer_balance + $paidValue);   
                     $payable_amount = ($payableAmount - $paidValue)- $customer_balance; 
                     $dateL=date("Y-m-d"); 
                    ...
                    */
                            ...
                            $cusLatestBill = $this->pos_model->getCustomerLatestBill($customer_id);
                            
                            //print_r($cusLatestBill);exit;  

                          if($cusLatestBill==false){

                                   $total_balance= $invoice_bill;    
                                   $transData = array(
                                          'customer_id'         => $customer_id,         
                                          'bill_amount'         => $invoice_bill,       
                                          'payment'             => $paidValue, 
                                          'paid_by'             => $paid_by, 
                                 ...
                                 $total_balance = ($cus_latest_bill + $invoice_bill) - $paidValue ;  
                          ...
                          $resultOF = $this->db->insert('transaction_history', $transData); 
                          $transaction_history_id = $this->db->insert_id();
                        
                           /* $cusBalance = $this->pos_model->getCustomerBalancebyID($customer_id);      
                             $payableAmount = $cusBalance->payable_amount + $invoice_bill; 
                         
                             $transData = array(
                                 'payable_amount'         => $payableAmount,          
                             );
                            $this->db->update('customer_balance', $transData, array('customer_id' => $customer_id));
                            
                            */
                          
                          //echo $transaction_history_id;
                          //echo '<br>'.$sale_id; exit; 
                          
                           $transDataUpdate = array(
                                 'transaction_history_id'         => $transaction_history_id,         
                             );
                            $this->db->update('sales', $transDataUpdate, array('id' => $sale_id));   
	...
	public function nsQTY($product_id, $quantity) {    
		...
		$this->db->update('products', array('quantity' => $nQTY, 'available_quantity' => $aQTY), array('id' => $product_id));
	}
	...
	*/
    ...
    public function getDiscountAmountByID($id, $product_id) {
        
        
       // echo "customer id".$id; 
       // echo "product id".$product_id; exit; 
        $myQuery = "Select * from customer_discount where customer_id=$id and product_id=$product_id";
        //$q = $this->db->get_where('customer_discount', array('customer_id' => $id, 'product_id' => $product_id), 1);
        $q = $this->db->query($myQuery, false);
	if( $q->num_rows() > 0 ){
            return $q->row();
        } else {
   ...
   public function getCustomerBalancebyID($custid) {     
        ...
        //$myQuery = "Select * from customer_balance where customer_id= $custid ";
       
     //$myQuery = "Select DISTINCT(customer_balance), date from customer_balance where customer_id= $custid ORDER BY date DESC LIMIT 1 ";
       
     $myQuery = "Select total_balance_amount from customer_balance where customer_id= $custid  LIMIT 1 ";
	...
	if( $q->num_rows() > 0 ){
          ...
          public function getOrderedProductByCodewithQuan($saleid, $code, $orderquan, $customerid)    
	{
                 $nq = $this->db->query("
                    Select `sale_items`.*, `products`.* from `sale_items`
                    left join `products`
                    ON `products`.code = `sale_items`.`product_code`
                    WHERE `sale_items`.sale_id=$saleid AND `sale_items`.product_code =$code
                    limit 1;"); 
                //$data = $query->result();
		//$q = $this->db->get_where('products', array('code' => $code), 1); 
		  if( $nq->num_rows() > 0 )
