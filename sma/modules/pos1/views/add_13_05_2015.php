    ...
    <body>
                    ...
                    <div class="pos">
                        ...
                        <div id="pos"> <?php echo form_open("module=pos"); ?>
                            <div id="leftdiv">
                                            ...
                                            <table id="title_table" border="0" cellpadding="0" cellspacing="0">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th style="width: 80px; color:#FFF;padding:5px 0; font-weight:normal;"><?php echo $this->lang->line('product'); ?></th>
                                                        <th style="width: 32px; color:#FFF;padding:5px 0; font-weight:normal;"><?php echo $this->lang->line('qty'); ?></th>
                                                        <th style="width: 80px; color:#FFF;padding:5px 0; font-weight:normal;"><?php echo 'Mainprice';?></th>
                                                        <th style="width: 72px; color:#FFF;padding:5px 0; font-weight:normal;"><?php echo $this->lang->line('discount'); ?></th>
                                ...
                                <input type="hidden" name="order_note" id="order_note" value="" style="display: none;" />
                ...
                <div class="well form-horizontal" style="margin-bottom:0;">
                    <div class="control-group" style="font-weight:bold;">
                       ...
                       <!--    
                        ...
                        <div id="currntbalncdiv" class="control-group">
                            <label class="control-label" for="curr_baln">Current Balance</label>
                            <div class="controls">                                               
                                <div id="currbaln" style="padding-top: 5px;"></div>             
                            </div>               
                        </div>            
                        <div class="pcash">
                            ...
                            <div class="control-group" style="font-weight:bold;">
                                ...
                                <div class="controls"><span style="background: #FFFF99; border-radius:5px; padding: 5px 10px; color: #000;" id="balance"></span></div>
                            </div>
                        </div>
                   -->
                    ...
                    <div class="pcheque" style="display:none;">
                        <div class="control-group">
                            <label class="control-label" for="paid-amount">
                                <?php echo $this->lang->line("cheque_no"); ?></label>
                            <div class="controls">
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $("#currntbalncdiv").hide();  
                var selCustomerID = "<?php echo $customer->id;?>";
                var count = 1;
                var total = 0;
...
<?php if (DISCOUNT_OPTION == 2) { ?>$("#ds").empty();
                ...
                $("#saletbl").on("click", 'button[class^="del_row"]', function() {
                    ...
                    var p1 = $('#price-' + rw_no);
                    //console.log(dl_id);
                    var res = rw_no.split("0");
                    console.log("#product-0"+res[1]);
                    var q1 = $('#quantity-' + rw_no);
                    $("#proajax #product-0"+res[1]).removeAttr("disabled");
                    $("#proajax #product-0"+res[1]).removeClass('btn-disabled');
                    $("#proajax #product-0"+res[1]).addClass('green');
<?php if (TAX1) { ?>
...
<?php } ?>
                    ...
                    var row_quantity = parseFloat(q1.val());
...
<?php } ?>
                ...
                function add_row() {
                    ...
                    $('button[id^="product"]').click(function() { 
                        ...
                        $("#proajax #"+this.id).attr('disabled','disabled');
                        $("#proajax #"+this.id).addClass('btn-disabled');
                        $("#proajax #"+this.id).removeClass('green');
                        $.ajax({
                            type: "get",
                            async: false,
                            url: "index.php?module=pos&view=price",
                            //data: '{ "code":' + v + ', "custid":'+selCustomerID+'}',
                            //data: {code: v},
                            data: {code: v, custid:selCustomerID},
                            dataType: "json",
                            success: function(data) {
                                item_price = parseFloat(data.price);
                                item_discount = parseFloat(data.discount_amount);
                                item_mainprice = parseFloat(data.original_price);
                                //item_price = parseFloat(data.price);
                ...
                function key_pad() {
                    $('.keyboard').keyboard({
                        ...
                        beforeClose: function(e, keyboard, el, accepted) {
                            ...
                            var id_no = sp_id[1];
                            var p = '#price-' + id_no;
                            var cidp = '#cusItemDiscountprice-' + id_no;
                            var itemizedisprice = parseFloat($.trim($(cidp).val()));
                            var row_price = parseFloat($.trim($(p).val()));
                            if (before_qty == 1) {
                                product_price = row_price;
                                totdisprice = itemizedisprice;
                            }
                            if (before_qty > 1) {
                                product_price = (row_price / before_qty);
                                totdisprice = (itemizedisprice/ before_qty);
                            }
                            ...
                            var cidpchngval = after_qty * totdisprice;
                            $(cidp).val(parseFloat(cidpchngval).toFixed(2));
...
<?php } ?>
                ...
                $("#customer").autocomplete({
                    source: function(request, response) {
                     ...
                     select: function(event, ui){
                            ...
                            selCustomerID = ui.item.id;
                            $("#saletbl").empty();
                            //$("button[id^="product-"]").removeClass('disabled'); 
                            //$("button[id^="product-"]").addClass('green'); 
                            clientchanged(selCustomerID);
                            $('.btn-disabled').prop("disabled", false);
                            $('.btn-disabled').addClass('green');
                            $('.btn-disabled').removeClass('btn-disabled');
                            //newTr.appendTo("#saletbl");
                            //console.log("selCustomerID :: "+selCustomerID);
                ...
                function clientchanged(selCustomerID){
                            $.ajax({
                                type: "post",  
                                async: false,
                                url: "index.php?module=pos&view=getCustomerBalance",
                                //data : "selectedcusid="+selCustomerID,
                                data: {selectedcusid:selCustomerID},
                                dataType: "json",
                                success: function(response) { 
                                    console.log(response);
                                    var customerbalance = response.balance; 
                                    $("#currntbalncdiv").show();
                                    $("#currbaln").text(customerbalance);     
                                },
                                error: function() {
                                    $("#currntbalncdiv").hide();
                                    $("#currbaln").text("");
...
<input type="hidden" name="serial' + count + '" value="" id="serial-' + count + slast + '">\n\
<input type="hidden" name="tax_rate' + count + '" value="' + pt + '" id="tax_rate-' + count + slast + '">\n\
<input type="hidden" name="discount' + count + '" value="<?php echo DEFAULT_DISCOUNT; ?>" id="discount-' + count + slast + '">\n\
<a href="#" id="model-' + count + slast + '" class="code">' + sproduct_name + '</a>\n\
<input type="hidden" name="price' + count + '" value="' + parseFloat(item_price).toFixed(2) + '" id="oprice-' + count + slast + '">\n\
</td><td style="text-align:center;">\n\
<input class="keyboard" onClick="this.select();" name="quantity' + count + '" type="text" value="1" autocomplete="off" id="quantity-' + count + slast + '"></td>\n\
<td style="padding-right: 10px; text-align:right;">\n\
<input type="text" class="price" name="unit_price' + count + '" value="' + parseFloat(item_price).toFixed(2) + '" id="price-' + count + slast + '"></td>');
                            newTr.appendTo("#saletbl");
...
<?php } ?>
                ...
                $("#payment").click(function() {
                    ...
                    $('#paymentModal').modal();
                    $("#ordernote").change(function() {
                        var p_val = $(this).val();
                        $('#order_note').val(p_val);
                        console.log("ordernote::" + p_val);
                    });
                    $("#paid_by").change(function() {
                        var p_val = $(this).val();
                        $('#rpaidby').val(p_val);
                        ...
                        success: function(data) {
                            if (data != null) {
                                ...
                                if (data.count != null) {
                                    ...
                                    sale_count = parseFloat(data.count);
                                } else {
                                    sale_count = 1;
                                }
                                if (data.tax1 != null) {
