...
*/
   ...
   function getdatatableajaxcost()
   {
	   ...
	   $this->datatables
			->select("products.id as productid, products.image as image, products.code as code, products.name as name, categories.name as cname, products.cost, products.price, COALESCE(quantity, 0) as quantity, products.unit, alert_quantity", FALSE)
                        //->select("products.id as productid, products.image as image, products.code as code, products.name as name, categories.name as cname, products.cost, products.price, COALESCE(available_quantity, 0) as available_quantity, products.unit, alert_quantity", FALSE)
			->from('products')
   ...
   function getdatatableajax()
   {
	   ...
	   $this->datatables
			...
			//->select("products.id as productid, products.image as image, products.code as code, products.name as name, categories.name as cname, products.price, COALESCE(available_quantity, 0) as available_quantity, products.unit, alert_quantity", FALSE);
	...
	function getdamageproducts()
   {
       ...
       function reset_stock()
	{
		...
		$result = $this->products_model->resetAllProductStock(); 
         ...
         function promotion_list()
   {
	  $data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
	  $data['success_message'] = $this->session->flashdata('success_message');
	  $data['product_promotion'] = $this->products_model->getAllProtion(); 
          //print_r($data['product_promotion']); exit;  
          $meta['page_title'] = 'Product Promotion';        
          $data['page_title'] = 'Product Promotion List';  
          $this->load->view('commons/header', $meta);          
          $this->load->view('promotion_list', $data);     
          $this->load->view('commons/footer');                
   }
   ...
   { 
		...
		$this->form_validation->set_rules('promotion_price', 'Price', 'required|numeric|xss_clean');
		...
		if ($this->form_validation->run() == true)
		{
			$data = array(
				...
				'promotion_price' => $this->input->post('promotion_price')
                        ...
                        $product_id=$this->input->post('product_id');
                        $result=$this->products_model->checkDuplicatePromotion($product_id);
                        
                        //print_r($result);exit; 
                        
                        if($result==false){  
                                                        
                            $this->products_model->addPromotion($data);  
                            $this->session->set_flashdata('success_message', 'Promotion Added');
                            redirect("module=products&view=promotion_list", 'refresh'); 
                        }
                        else
                        {

                            //echo $product_id; exit; 
                            $productData=$this->products_model->getDuplicatePromotionData($product_id); 
                            $promotion_id=$productData->promotion_id;
                            $this->products_model->updatePromotion($promotion_id, $data);  
                            $this->session->set_flashdata('success_message', 'Promotion Updated');
                            redirect("module=products&view=promotion_list", 'refresh'); 
                        }
		}
                else
                ...
                $data['success_message'] = $this->session->flashdata('success_message');
                $meta['page_title'] = 'Product Promotion';
                $data['page_title'] = 'Product Promotion'; 
                $this->load->view('commons/header', $meta);
    ...
    { 
		...
		if ( $this->form_validation->run() == true && $this->products_model->updateProductPromotion($id, $data))
		{ 
			$this->session->set_flashdata('success_message', 'Promotion Updated'); 
			redirect("module=products&view=promotion_list", 'refresh'); 
		}
		else
		{
                ...
                $data['product_promotion'] = $this->products_model->getProductPromotionByID($id);  
                
                //print_r($data['customer_discount']); exit;  
                
                $meta['page_title'] = 'Product Promotion Update';
                $data['page_title'] = 'Product Promotion Update'; 
                $this->load->view('commons/header', $meta);
                $this->load->view('edit_product_promotion',$data);  
                $this->load->view('commons/footer');
        ...
        function delete_promotion($id = NULL)
	{
		...
		if ( $this->products_model->deleteProductPromotion($id) )
		{ 
			$this->session->set_flashdata('success_message', 'Promotion Deleted');
			redirect('module=products&view=promotion_list', 'refresh');
		}
