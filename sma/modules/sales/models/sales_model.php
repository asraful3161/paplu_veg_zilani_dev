...
class Sales_model extends CI_Model {
    ...
    public function deleteInvoice($id) {
        ...
        $invoiceData=$this->sales_model->getInvoiceDataFromTranHistoryBYID($id); 
        $transId=$invoiceData->transaction_history_id;
        $customerId=$invoiceData->customer_id;
        $invoiceBill=$invoiceData->bill_amount;
        $latestPayment=$invoiceData->latest_payment;
        $preBalance=$invoiceData->pre_balance;
        
       
        
        if ($this->db->delete('sale_items', array('sale_id' => $id)) && $this->db->delete('transaction_history', array('transaction_history_id' => $transId)) && $this->db->delete('sales', array('id' => $id))) {
            
                   $transIdData=$this->sales_model->getAllTransIdLessThanSalesTransId($transId,$customerId); 
                   //print_r($transIdData); exit;   
                   $numberOFTransID=count($transIdData);

                   if($transIdData != ''){

                     for($i=0; $i<=$numberOFTransID-1; $i++) {

                       $transIdForUpdate=$transIdData[$i]['transaction_history_id']; 

                       $dataOfTransID=$this->sales_model->getdataOfTransId($transIdForUpdate);
                        $currTotalBalance=$dataOfTransID->total_balance; 
                        $currPreBalance=$dataOfTransID->pre_balance;	

                        $newTotalBalance=$currTotalBalance - $invoiceBill; 
                        $newPreBalance=$currPreBalance - $invoiceBill; 

                       $this->sales_model->updateTransId($newTotalBalance,$newPreBalance,$transIdForUpdate);

                       }

                   }
            
                     $cusLatestBill = $this->sales_model->getCustomerLatestBill($customerId);       
                     $cus_latest_bill=$cusLatestBill->total_balance;            
                     $balanceData = array(
				'total_balance_amount'  => $cus_latest_bill,   
			);
                     $this->db->update('customer_balance', $balanceData, array('customer_id' => $customerId));
...
//////////////////////////// For Delete start ///////////////////////////////
   ...
   public function getInvoiceDataFromTranHistoryBYID($sale_id) {  
     ...
     public function getAllTransIdLessThanSalesTransId($transId,$customerId) {  

        $myQuery = "SELECT transaction_history_id
                    FROM transaction_history
                    WHERE customer_id=$customerId
                    AND transaction_history_id > $transId 
                    ";
  ...
  public function getdataOfTransId($transIdForUpdate) {  

        $myQuery = "SELECT total_balance, pre_balance 		
                    FROM transaction_history
                    WHERE transaction_history_id = $transIdForUpdate 
                    ";
    ...
    public function updateTransId($newTotalBalance,$newPreBalance,$transIdForUpdate){
        
        //echo $transIdForUpdate; exit; 
        
        $this->db->update('transaction_history', array('total_balance' => $newTotalBalance,'pre_balance' => $newPreBalance ), array('transaction_history_id' => $transIdForUpdate));
    }
...
//        echo '</pre>';         
        ...
        $cusLatestBill = $this->getCustomerLatestBill($customer_id);
        $cus_latest_bill=$cusLatestBill->total_balance; 
        $transactionHistoryId=$cusLatestBill->transaction_history_id; 
           
        $InvoiceInfo  = $this->getInvoiceInfo($saleID);
        
        $previous_bill_amount = $InvoiceInfo->bill_amount;
        $invoice_total_balance = $InvoiceInfo->total_balance; 
        $invoice_pre_balance = $InvoiceInfo->pre_balance; 
        $transaction_history_id = $InvoiceInfo->transaction_history_id;
         
        $diff_amount = $previous_bill_amount - $newinv_total;
        $finalBalance = $invoice_total_balance - $diff_amount;   
        
        $updatedate = date("Y-m-d"); 
        
        $upadateData = array(
                            'bill_amount'	=> $newinv_total,  
                            'total_balance'	=> $finalBalance,  
                            'update_date'	=> $updatedate,  
                            );
        $result=$this->db->update('transaction_history', $upadateData, array('sale_id' => $saleID, 'customer_id' => $customer_id ));
        
          if($result){    
            $custransInfo = $this->getCustomerTrans($transaction_history_id,$customer_id);
            if(!empty($custransInfo)){
                $numOfTransId=count($custransInfo);
                foreach ($custransInfo as $key => $value) {
                    $data=array(
                      'pre_balance'=>$value['pre_balance']-$diff_amount,
                      'total_balance'=>$value['total_balance']-$diff_amount 
                    );
                   $this->db->update('transaction_history', $data, array('customer_id' => $customer_id, 'transaction_history_id' => $value['transaction_history_id']));            
                }
            }
                $cusLatestBalanceData=$this->sales_model->getCustomerLatestBill($customer_id);  
                $customeLatestBalance=$cusLatestBalanceData->total_balance;  
                $cusBalanceData = array(
                                'total_balance_amount'		=> $customeLatestBalance,  
                                'date'	 		        => $updatedate,  
                                );
    ...
    public function getCustomerTrans($transaction_history_id,$customer_id){     
 
     $myQuery = "Select * FROM transaction_history WHERE customer_id=$customer_id AND transaction_history_id > $transaction_history_id ";
