...
//index or inventories page
    ...
    function getdatatableajax() {
        ...
        $this->datatables->add_column("Actions", "<center><a href='#' title='$2' class='tip' data-html='true'><i class='icon-folder-close'></i></a> <a href='#' onClick=\"MyWindow=window.open('index.php?module=sales&view=view_invoice&id=$1', 'MyWindow','toolbar=0,location=0,directories=0,status=0,menubar=yes,scrollbars=yes,resizable=yes,width=1000,height=600'); return false;\" title='" . $this->lang->line("view_invoice") . "' class='tip'><i class='icon-fullscreen'></i></a> 
			...
			<a href='index.php?module=sales&view=email_invoice&id=$1' title='" . $this->lang->line("email_invoice") . "' class='tip'><i class='icon-envelope'></i></a>
                        ...
                        <a href='index.php?module=sales&amp;view=view_pos_invoice&amp;id=$1' title='" . 'Pos Invoice' . "' class='tip'><i class='icon-book'></i></a>
    ...
    function getwhinv($warehouse_id = NULL) {
        ...
        $this->datatables
                ...
                ->add_column("Actions", "<center><a href='#' onClick=\"MyWindow=window.open('index.php?module=sales&view=view_invoice&id=$1', 'MyWindow','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=1000,height=600'); return false;\" title='" . $this->lang->line("view_invoice") . "' class='tip'><i class='icon-fullscreen'></i></a> 
			<a href='index.php?module=sales&view=add_delivery&id=$1' title='" . $this->lang->line("add_delivery_order") . "' class='tip'><i class='icon-road'></i></a>
			<a href='index.php?module=sales&view=pdf&id=$1' title='" . $this->lang->line("download_pdf") . "' class='tip'><i class='icon-file'></i></a> 
			<a href='index.php?module=sales&view=email_invoice&id=$1' title='" . $this->lang->line("email_invoice") . "' class='tip'><i class='icon-envelope'></i></a>
			<a href='index.php?module=sales&amp;view=edit&amp;id=$1' title='" . $this->lang->line("edit_invoice") . "' class='tip'><i class='icon-edit'></i></a>
...
//    }
    function view_invoice() {
        ...
        $cusBalance = $this->sales_model->getCustomerBalancebyID($customer_id);
        $data['cusLatestAmount'] = $cusBalance->total_balance_amount;
        //print_r($cusBalance);exit;

        $cusLatestBalance = $this->sales_model->getCustomerLatestBalancebyID($customer_id);
        $data['latestBalanceAmount'] = $cusLatestBalance->total_balance;
        
        $cusPreBalance = $this->sales_model->getCustomerPreBalancebyID($sale_id,$customer_id);
        $data['cusPreBalance'] = $cusPreBalance->pre_balance;

        //print_r($cusPreBalance);exit; 
        ...
        //$cusLatestSalData=$this->sales_model->getCustomerLatestSaleID($customer_id);
        //$cusLatestSaleid=$cusLatestSalData->transaction_history_id;
        
        //$cusLatestPayment = $this->sales_model->getCustomerLatestPaymentID($customer_id);
        //$transIdofLatestPayment=$cusLatestPayment->transaction_history_id; 
         
        
        $dataforTransaction=$this->sales_model->getPaymentForInvoice($sale_id);
        $data['latestPaymentmount'] = $dataforTransaction->latest_payment;
        
        
       //$dataforTransaction=$this->sales_model->getTransIdofInvoice($sale_id);
       // $transIdofInvoice=$dataforTransaction->transaction_history_id; 
        
       /* if($transIdofInvoice > $transIdofLatestPayment){ 
           $data['latestPaymentmount'] = 0.00; 
        }else{
           $data['latestPaymentmount'] = $cusLatestPayment->payment; 
        }
...
//            echo "</pre>"; 
            ...
            for ($i = 1; $i <= 500; $i++) {
                if ($this->input->post($quantity . $i) && $this->input->post($product . $i) && $this->input->post($unit_price . $i)) {
                    ...
                    $product_unit[] = $product_details->unit;
                    $inv_unit_price[] = $this->input->post($unit_price . $i);
                    $_variprod_note = $this->input->post($var_poductnote . $i);
                    $variprod_note[] = $_variprod_note;
                    $prweight = $this->sales_model->getCustomerProductWeight($product_details->id);
                    $_product_type = $product_details->cf1;
                    $_inv_gross_total = 0;
                    if ($_product_type === "Variable" && !empty($_variprod_note)) {
                        $_variprod_note_arr = explode("+", $_variprod_note);
                        //print_r($_variprod_note_arr);
                        if (!empty($_variprod_note_arr)) {
                            $nwtotweight = 0;
                            foreach ($_variprod_note_arr as $value) {
                                $nwtotweight += $value;
                            }
                            if (empty($prweight) || $prweight == 0) {
                                $prweight = 1;
                            }
                            $_unit_price = $this->input->post($unit_price . $i);
                            $_inv_gross_total = ($_unit_price / $prweight) * $nwtotweight;
                            $inv_gross_total[] = $_inv_gross_total;
                            $sale_product_weight[] = $nwtotweight;
                        }
                    } else {
                        $_inv_gross_total = (($this->input->post($quantity . $i)) * ($this->input->post($unit_price . $i)));
                        $inv_gross_total[] = (($this->input->post($quantity . $i)) * ($this->input->post($unit_price . $i)));
                        $sale_product_weight[] = (($this->input->post($quantity . $i)) * $prweight);
                    }
                    $inv_total_no_tax += $_inv_gross_total;
                    //$inv_total_no_tax += (($this->input->post($quantity . $i)) * ($this->input->post($unit_price . $i)));                   
                }
            }
            //exit;
            ...
            $keys = array("product_id", "product_code", "product_name", "product_unit", "tax_rate_id", "tax", "quantity", "sale_product_weight", "variprod_note", "unit_price", "gross_total", "val_tax", "serial_no", "discount_val", "discount", "discount_id");

            $items = array();
            foreach (array_map(null, $product_id, $product_code, $product_name, $product_unit, $tax_rate_id, $tax, $inv_quantity, $sale_product_weight, $variprod_note, $inv_unit_price, $inv_gross_total, $val_tax, $serial, $val_ds, $discount, $dsID) as $key => $value) {
                $items[] = array_combine($keys, $value);
            }
        ...
        if ($this->form_validation->run() == true && !empty($items)) {
            if ($this->sales_model->updateSale($id, $saleDetails, $items, $warehouse_id)) {
                $this->sales_model->updateTranHistory($id, $saleDetails);
    ...
    function getdeliveries() {
        ...
        $this->datatables
                ...
                ->from('deliveries');
        $this->datatables->add_column("Actions", "<center><a href='#' onClick=\"MyWindow=window.open('index.php?module=sales&view=view_delivery&id=$1', 'MyWindow','toolbar=0,location=0,directories=0,status=0,menubar=yes,scrollbars=yes,resizable=yes,width=1000,height=600'); return false;\" title='" . $this->lang->line("view_delivery") . "' class='tip'><i class='icon-fullscreen'></i></a> 
    ...
    function scan_item() {
        ...
        if ($item = $this->sales_model->getProductByCode($code)) {
            ...
            $code = $item->code;
            $weight = $item->weight;
            $producttype = $item->cf1;
            //$product_tax = $item->tax_rate;

            $tax_rate = $this->sales_model->getTaxRateByID($product_tax);

            $product = array('name' => $product_name, 'code' => $code, 'price' => $product_price, 'tax_rate' => $tax_rate, 'weight' => $weight, 'product_type' => $producttype);
    ...
    function add_item() {
        ...
        if ($item = $this->sales_model->getProductByName($name)) {
            ...
            $product = array('code' => $code, 'price' => $price, 'tax_rate' => $tax_rate, 'weight' => $weight, 'product_type' => $producttype);
        }
    ...
    function view_pos_invoice() {

              if($this->input->get('id')){ $sale_id = $this->input->get('id'); } else { $sale_id = NULL; }
 
              $data['message'] = (validation_errors() ? validation_errors() : $this->session->flashdata('message'));

               $data['rows'] = $this->sales_model->getAllInvoiceItems_old($sale_id);

               $inv = $this->sales_model->getInvoiceBySaleID_old($sale_id);
               $biller_id = $inv->biller_id;
               $customer_id = $inv->customer_id;
               $invoice_type_id = $inv->invoice_type;
               $data['biller'] = $this->sales_model->getBillerByID($biller_id);
               $data['customer'] = $this->sales_model->getCustomerByID($customer_id);
               $data['invoice_types_details'] = $this->sales_model->getInvoiceTypeByID($invoice_type_id);
               $data['pos'] = $this->sales_model->getSetting();
               
               $data['cusBalance'] = $this->sales_model->getCustomerBalancebyID($customer_id);
               ...
               $data['page_title'] = $this->lang->line("invoice"); 
               $this->load->view('view_pos_invoice', $data);
