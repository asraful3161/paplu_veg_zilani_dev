        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($i = 1; $i <= 500; $i++) {
                if ($this->input->post($quantity . $i) && $this->input->post($product . $i) && $this->input->post($unit_price . $i)) {
                    ...
                    $product_code[] = $product_details->code;
                    $product_unit[] = $product_details->unit;
                    //$product_total_weight[] = $product_details->total_weight;
                    $inv_unit_price[] = $this->input->post($unit_price . $i);
                    $inv_gross_total[] = (($this->input->post($quantity . $i)) * ($this->input->post($unit_price . $i)));
            ...
            $keys = array("product_id", "product_code", "product_name", "product_unit", "tax_rate_id", "tax", "quantity", "total_weight", "unit_price", "gross_total", "val_tax", "serial_no", "discount_val", "discount", "discount_id");
